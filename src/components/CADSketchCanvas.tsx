...
export const CADSketchCanvas: React.FC<CADSketchCanvasProps> = ({
  ...
  const [drawPoints, setDrawPoints] = useState<Point2D[]>([]); // 新增：用於記錄圓弧等多點繪製
  ...
  const handleCanvasClick = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (currentTool === 'LINE') {
      ...
      if (!isDrawing || !drawStartPt) {
        ...
        const dx = Math.abs(clickPt.x - drawStartPt.x);
        const dy = Math.abs(clickPt.y - drawStartPt.y);
        
        if (dx > 0.5 && dy > 0.5) {
          const newRect: PolylineEntity = {
            id: `ent_rect_${Date.now().toString().slice(-6)}`,
            type: 'polyline',
            layer: 'layer_outline',
            color: '#38bdf8',
            state: 'under_constrained',
            ...
            vertices: [
              { point: { ...drawStartPt } },
              { point: { x: clickPt.x, y: drawStartPt.y } },
              { point: { ...clickPt } },
              { point: { x: drawStartPt.x, y: clickPt.y } }
            ]
          };
          addEntity(targetSketchId, newRect);
          setSelectedEntityIds([newRect.id]);
        }
        setIsDrawing(false);
        setDrawStartPt(null);
      }
      ...
      if (drawPoints.length === 0) {
        ...
        setDrawPoints([...drawPoints, clickPt]);
      } else if (drawPoints.length === 2) {
        const center = drawPoints[0];
        const startPt = drawPoints[1];
        ...
        if (radius > 1) {
          ...
          const endAngle = Math.atan2(clickPt.y - center.y, clickPt.x - center.x);

          const newArc: ArcEntity = {
            id: `ent_arc_${Date.now().toString().slice(-6)}`,
            type: 'arc',
            layer: 'layer_outline',
            color: '#38bdf8',
            state: 'under_constrained',
          ...
          addEntity(targetSketchId, newArc);
          setSelectedEntityIds([newArc.id]);
        }
        setIsDrawing(false);
        setDrawPoints([]);
      }
  ...
  if (isDrawing && cursorWorld) {
    if (drawStartPt) {
      rubberBandDist = Math.hypot(cursorWorld.x - drawStartPt.x, cursorWorld.y - drawStartPt.y);
    } else if (drawPoints.length > 0) {
      const lastPt = drawPoints[drawPoints.length - 1];
      rubberBandDist = Math.hypot(cursorWorld.x - lastPt.x, cursorWorld.y - lastPt.y);
    }
  }
        ...
        <div className="flex items-center gap-3">
          ...
          {isDrawing && (
            <div className="flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-amber-950/80 border border-amber-700/70 text-amber-300 text-[11px] animate-pulse">
              ...
              <span>
                {currentTool === 'LINE' ? 'Drawing Line (Click endpoint)' :
                 currentTool === 'CIRCLE' ? 'Drawing Circle (Click radius)' :
                 currentTool === 'RECTANGLE' ? 'Drawing Rectangle (Click opposite corner)' :
                 currentTool === 'ARC' ? (drawPoints.length === 1 ? 'Select arc start point' : 'Select arc end point') :
                 'Drawing...'}
              </span>
      ...
      <div className="relative flex-1 bg-slate-950 min-h-[380px] overflow-hidden select-none">
        ...
        >
          ...
          <defs>
            ...
            <marker id="dim-arrow-start" viewBox="0 0 10 10" refX="1" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              <polygon points="10,2 1,5 10,8" fill="#10b981" />
            </marker>
            <marker id="dim-arrow-end" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto">
              <polygon points="1,2 9,5 1,8" fill="#10b981" />
            </marker>

            <marker id="dim-preview-arrow-start" viewBox="0 0 10 10" refX="1" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse">
              <polygon points="10,2 1,5 10,8" fill="#f59e0b" />
            </marker>
            <marker id="dim-preview-arrow-end" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="6" markerHeight="6" orient="auto">
          ...
          {/* 座標原點 */}
          <g>
            <line x1={toSvgX(0)} y1={toSvgY(0)} x2={toSvgX(25)} y2={toSvgY(0)} stroke="#ef4444" strokeWidth="2" />
            <polygon points={`${toSvgX(25)},${toSvgY(0) - 3} ${toSvgX(29)},${toSvgY(0)} ${toSvgX(25)},${toSvgY(0) + 3}`} fill="#ef4444" />
            <text x={toSvgX(32)} y={toSvgY(0) + 4} fill="#ef4444" fontSize="10" fontWeight="bold">X</text>

            <line x1={toSvgX(0)} y1={toSvgY(0)} x2={toSvgX(0)} y2={toSvgY(25)} stroke="#22c55e" strokeWidth="2" />
            <polygon points={`${toSvgX(0) - 3},${toSvgY(25)} ${toSvgX(0)},${toSvgY(29)} ${toSvgX(0) + 3},${toSvgY(25)}`} fill="#22c55e" />
            <text x={toSvgX(0) - 4} y={toSvgY(32)} fill="#22c55e" fontSize="10" fontWeight="bold">Y</text>

            <circle cx={toSvgX(0)} cy={toSvgY(0)} r="3.5" fill="#fbbf24" stroke="#0f172a" strokeWidth="1.5" />
            <text x={toSvgX(0) - 18} y={toSvgY(0) + 16} fill="#94a3b8" fontSize="9" fontFamily="monospace">(0, 0)</text>
          </g>
              ...
              profiles.map((prof) => {
                ...
                return (
                  <polygon
                    ...
                    points={ptsString}
                    fill={prof.isIsland ? 'rgba(244, 63, 94, 0.08)' : 'rgba(56, 189, 248, 0.12)'}
                    stroke={prof.isIsland ? 'rgba(244, 63, 94, 0.35)' : 'rgba(56, 189, 248, 0.4)'}
                    strokeWidth="1.2"
                    strokeDasharray="4,3"
                  />
                );
          ...
          {entities.map((entity) => {
            ...
            const strokeColor = isSelected ? '#38bdf8' : isDimTarget ? '#c084fc' : isHovered ? '#f59e0b' : isFullyDefined ? '#ffffff' : entity.isConstruction ? '#a855f7' : entity.color || '#38bdf8';
            const pointFill = isSelected ? '#38bdf8' : isDimTarget ? '#c084fc' : isHovered ? '#f59e0b' : isFullyDefined ? '#ffffff' : strokeColor;
            ...
            if (entity.type === 'line') {
              return (
                <g key={entity.id} className="cursor-pointer" onMouseEnter={() => !isDrawing && setHoveredEntityId(entity.id)} onMouseLeave={() => setHoveredEntityId(null)}>
                  <line x1={toSvgX(entity.start.x)} y1={toSvgY(entity.start.y)} x2={toSvgX(entity.end.x)} y2={toSvgY(entity.end.y)} stroke={strokeColor} strokeWidth={strokeWidth} strokeDasharray={strokeDasharray} />
                  <circle cx={toSvgX(entity.start.x)} cy={toSvgY(entity.start.y)} r={isSelected ? 4.5 : 3} fill={pointFill} />
                  <circle cx={toSvgX(entity.end.x)} cy={toSvgY(entity.end.y)} r={isSelected ? 4.5 : 3} fill={pointFill} />
                </g>
              );
            }
            ...
            if (entity.type === 'arc') {
              ...
              return (
                <g key={entity.id} className="cursor-pointer" onMouseEnter={() => !isDrawing && setHoveredEntityId(entity.id)} onMouseLeave={() => setHoveredEntityId(null)}>
                  ...
                  <circle cx={toSvgX(entity.center.x)} cy={toSvgY(entity.center.y)} r={isSelected ? 3.5 : 2.5} fill={isSelected ? '#38bdf8' : isFullyDefined ? '#ffffff' : '#f59e0b'} />
                </g>
              );
            }
            ...
            if (entity.type === 'circle') {
              return (
                <g key={entity.id} className="cursor-pointer" onMouseEnter={() => !isDrawing && setHoveredEntityId(entity.id)} onMouseLeave={() => setHoveredEntityId(null)}>
                  <circle cx={toSvgX(entity.center.x)} cy={toSvgY(entity.center.y)} r={entity.radius * scale} fill={isSelected ? 'rgba(56, 189, 248, 0.16)' : isFullyDefined ? 'rgba(255, 255, 255, 0.05)' : 'rgba(239, 68, 68, 0.08)'} stroke={strokeColor} strokeWidth={strokeWidth} />
                  <circle cx={toSvgX(entity.center.x)} cy={toSvgY(entity.center.y)} r={isSelected ? 4 : 3} fill={isSelected ? '#38bdf8' : isFullyDefined ? '#ffffff' : '#ef4444'} />
                </g>
              );
            }

            if (entity.type === 'polyline') {
              const pointsStr = entity.vertices.map((v) => `${toSvgX(v.point.x)},${toSvgY(v.point.y)}`).join(' ');
              return (
                <g key={entity.id} className="cursor-pointer" onMouseEnter={() => !isDrawing && setHoveredEntityId(entity.id)} onMouseLeave={() => setHoveredEntityId(null)}>
                  <polyline points={pointsStr} fill="none" stroke={strokeColor} strokeWidth={strokeWidth} strokeDasharray={strokeDasharray} />
                  {entity.vertices.map((v, i) => (
                    <circle key={i} cx={toSvgX(v.point.x)} cy={toSvgY(v.point.y)} r={isSelected ? 3.5 : 2.5} fill={pointFill} />
                  ))}
                </g>
              );
            }
          ...
          {/* 橡皮筋動態預覽 */}
          {isDrawing && cursorWorld && (
            <g id="rubber-band-preview-group" pointerEvents="none">
              {currentTool === 'LINE' && drawStartPt && (
                <>
                  <line x1={toSvgX(drawStartPt.x)} y1={toSvgY(drawStartPt.y)} x2={toSvgX(cursorWorld.x)} y2={toSvgY(cursorWorld.y)} stroke="#f59e0b" strokeWidth="2" strokeDasharray="5,5" />
                  <circle cx={toSvgX(drawStartPt.x)} cy={toSvgY(drawStartPt.y)} r="4" fill="#f59e0b" stroke="#1e293b" strokeWidth="1.5" />
                  <circle cx={toSvgX(cursorWorld.x)} cy={toSvgY(cursorWorld.y)} r="3.5" fill="#fbbf24" />
                  <g transform={`translate(${(toSvgX(drawStartPt.x) + toSvgX(cursorWorld.x)) / 2}, ${(toSvgY(drawStartPt.y) + toSvgY(cursorWorld.y)) / 2 - 12})`}>
                    <rect x="-30" y="-9" width="60" height="16" rx="3" fill="#451a03" stroke="#d97706" strokeWidth="1" />
                    <text x="0" y="3" textAnchor="middle" fill="#fef08a" fontSize="9" fontFamily="monospace" fontWeight="bold">{rubberBandDist.toFixed(1)} mm</text>
                  </g>
                </>
              )}

              {currentTool === 'CIRCLE' && drawStartPt && (
                <>
                  <circle cx={toSvgX(drawStartPt.x)} cy={toSvgY(drawStartPt.y)} r={Math.max(rubberBandDist * scale, 1)} fill="rgba(245, 158, 11, 0.08)" stroke="#f59e0b" strokeWidth="2" strokeDasharray="5,5" />
                  <line x1={toSvgX(drawStartPt.x)} y1={toSvgY(drawStartPt.y)} x2={toSvgX(cursorWorld.x)} y2={toSvgY(cursorWorld.y)} stroke="#d97706" strokeWidth="1" strokeDasharray="2,2" />
                  <circle cx={toSvgX(drawStartPt.x)} cy={toSvgY(drawStartPt.y)} r="4" fill="#ef4444" stroke="#1e293b" strokeWidth="1.5" />
                  <circle cx={toSvgX(cursorWorld.x)} cy={toSvgY(cursorWorld.y)} r="3.5" fill="#f59e0b" />
                  <g transform={`translate(${(toSvgX(drawStartPt.x) + toSvgX(cursorWorld.x)) / 2}, ${(toSvgY(drawStartPt.y) + toSvgY(cursorWorld.y)) / 2 - 12})`}>
                    <rect x="-28" y="-9" width="56" height="16" rx="3" fill="#451a03" stroke="#d97706" strokeWidth="1" />
                    <text x="0" y="3" textAnchor="middle" fill="#fef08a" fontSize="9" fontFamily="monospace" fontWeight="bold">R {rubberBandDist.toFixed(1)}</text>
                  </g>
                </>
              )}

              {currentTool === 'RECTANGLE' && drawStartPt && (
                <rect
                  x={Math.min(toSvgX(drawStartPt.x), toSvgX(cursorWorld.x))}
                  y={Math.min(toSvgY(drawStartPt.y), toSvgY(cursorWorld.y))}
                  width={Math.abs(toSvgX(cursorWorld.x) - toSvgX(drawStartPt.x))}
                  height={Math.abs(toSvgY(cursorWorld.y) - toSvgY(drawStartPt.y))}
              ...
              {currentTool === 'ARC' && drawPoints.length > 0 && (
                <>
                  {drawPoints.length === 1 && (
                     <line x1={toSvgX(drawPoints[0].x)} y1={toSvgY(drawPoints[0].y)} x2={toSvgX(cursorWorld.x)} y2={toSvgY(cursorWorld.y)} stroke="#d97706" strokeWidth="1" strokeDasharray="2,2" />
                  )}
                  {drawPoints.length === 2 && (() => {
                     const center = drawPoints[0];
                     const startPt = drawPoints[1];
                     ...
                     return (
                       <circle cx={toSvgX(center.x)} cy={toSvgY(center.y)} r={r * scale} fill="none" stroke="#f59e0b" strokeWidth="2" strokeDasharray="5,5" />
                     );
                  })()}
                </>
              )}
            ...
            <g id="cad-dimensions-layer" className="select-none font-mono">
              {dimensions.map((dim) => {
                ...
                if (targetEnt && targetEnt.type === 'line') {
                  p1 = targetEnt.start; p2 = targetEnt.end;
                } else if (dim.type === 'linear_horizontal') {
                  p1 = { x: 0, y: 0 }; p2 = { x: 100, y: 0 };
                } else if (dim.type === 'linear_vertical') {
                  p1 = { x: 0, y: 0 }; p2 = { x: 0, y: 80 };
                }

                if (!p1 || !p2) {
                  const tx = toSvgX(dim.textPosition.x); const ty = toSvgY(dim.textPosition.y);
                  return (
                    <g key={dim.id}>
                      <rect x={tx - 26} y={ty - 9} width="52" height="18" rx="3" fill="#022c22" stroke="#059669" strokeWidth="1.2" />
                      <text x={tx} y={ty + 3.5} textAnchor="middle" fill="#34d399" fontSize="9.5" fontWeight="bold">{dim.prefix || ''}{dim.value}{dim.suffix || ' mm'}</text>
                    </g>
                  );
                }
                ...
                return (
                  <g key={dim.id} id={`dim-${dim.id}`} className="transition-opacity">
                    <line x1={toSvgX(geom.a1.x)} y1={toSvgY(geom.a1.y)} x2={toSvgX(geom.b1.x)} y2={toSvgY(geom.b1.y)} stroke="#10b981" strokeWidth="1" strokeDasharray="3,2" opacity="0.8" />
                    <line x1={toSvgX(geom.a2.x)} y1={toSvgY(geom.a2.y)} x2={toSvgX(geom.b2.x)} y2={toSvgY(geom.b2.y)} stroke="#10b981" strokeWidth="1" strokeDasharray="3,2" opacity="0.8" />
                    <line x1={toSvgX(geom.d1.x)} y1={toSvgY(geom.d1.y)} x2={toSvgX(geom.d2.x)} y2={toSvgY(geom.d2.y)} stroke="#10b981" strokeWidth="1.2" markerStart="url(#dim-arrow-start)" markerEnd="url(#dim-arrow-end)" />
                    <g transform={`translate(${toSvgX(geom.textCenter.x)}, ${toSvgY(geom.textCenter.y)})`}>
                      <rect x="-28" y="-9" width="56" height="18" rx="3" fill="#022c22" stroke="#059669" strokeWidth="1.2" />
                      <text x="0" y="3.5" textAnchor="middle" fill="#34d399" fontSize="9.5" fontWeight="bold">{dim.prefix || ''}{dim.value}{dim.suffix || ' mm'}</text>
                    </g>
                  </g>
                );
              })}
              ...
              {currentTool === 'DIMENSION' && dimensionTargetLine && cursorWorld && (() => {
                const p1 = dimensionTargetLine.start; const p2 = dimensionTargetLine.end;
                const geom = computeDimensionGeometry(p1, p2, cursorWorld);
                if (!geom) return null;
                const previewVal = (Math.round(Math.hypot(p2.x - p1.x, p2.y - p1.y) * 10) / 10).toFixed(1);

                return (
                  <g id="dimension-placement-preview" pointerEvents="none">
                    <line x1={toSvgX(geom.a1.x)} y1={toSvgY(geom.a1.y)} x2={toSvgX(geom.b1.x)} y2={toSvgY(geom.b1.y)} stroke="#f59e0b" strokeWidth="1" strokeDasharray="3,2" opacity="0.9" />
                    <line x1={toSvgX(geom.a2.x)} y1={toSvgY(geom.a2.y)} x2={toSvgX(geom.b2.x)} y2={toSvgY(geom.b2.y)} stroke="#f59e0b" strokeWidth="1" strokeDasharray="3,2" opacity="0.9" />
                    <line x1={toSvgX(geom.d1.x)} y1={toSvgY(geom.d1.y)} x2={toSvgX(geom.d2.x)} y2={toSvgY(geom.d2.y)} stroke="#f59e0b" strokeWidth="1.3" markerStart="url(#dim-preview-arrow-start)" markerEnd="url(#dim-preview-arrow-end)" />
                    <g transform={`translate(${toSvgX(geom.textCenter.x)}, ${toSvgY(geom.textCenter.y)})`}>
                      <rect x="-30" y="-10" width="60" height="20" rx="4" fill="#451a03" stroke="#d97706" strokeWidth="1.5" />
                      <text x="0" y="3.5" textAnchor="middle" fill="#fef08a" fontSize="9.5" fontWeight="bold">{previewVal} mm</text>
                    </g>
                  </g>
                );
              })()}
          ...
          {showConstraints && (
            <g>
              <g transform={`translate(${toSvgX(50)}, ${toSvgY(0) + 12})`}><rect x="-8" y="-7" width="16" height="14" rx="2" fill="#1e1b4b" stroke="#6366f1" strokeWidth="1" /><text x="0" y="3" textAnchor="middle" fill="#a5b4fc" fontSize="9" fontWeight="bold">—</text></g>
              <g transform={`translate(${toSvgX(0) - 14}, ${toSvgY(40)})`}><rect x="-7" y="-8" width="14" height="16" rx="2" fill="#1e1b4b" stroke="#6366f1" strokeWidth="1" /><text x="0" y="4" textAnchor="middle" fill="#a5b4fc" fontSize="9" fontWeight="bold">|</text></g>
              <g transform={`translate(${toSvgX(80)}, ${toSvgY(80) - 12})`}><circle r="7" fill="#1e1b4b" stroke="#6366f1" strokeWidth="1" /><text x="0" y="3" textAnchor="middle" fill="#a5b4fc" fontSize="8" fontWeight="bold">tan</text></g>
              <g transform={`translate(${toSvgX(0) - 10}, ${toSvgY(0) - 10})`}><rect x="-6" y="-6" width="12" height="12" rx="2" fill="#451a03" stroke="#d97706" strokeWidth="1" /><text x="0" y="3.5" textAnchor="middle" fill="#fcd34d" fontSize="8" fontWeight="bold">☩</text></g>
            </g>
          )}

          {/* OSnap Visual Markers */}
          {currentSnap && (
            <g id="osnap-marker-layer" pointerEvents="none" className="select-none">
              {(() => {
                ...
                return (
                  <g>
                    {currentSnap.type === 'endpoint' && <rect id="osnap-endpoint-marker" x={snapSvgX - 5} y={snapSvgY - 5} width={10} height={10} fill="none" stroke="#22c55e" strokeWidth="2" />}
                    {currentSnap.type === 'midpoint' && <polygon id="osnap-midpoint-marker" points={`${snapSvgX},${snapSvgY - 6.5} ${snapSvgX - 6},${snapSvgY + 5} ${snapSvgX + 6},${snapSvgY + 5}`} fill="none" stroke="#22c55e" strokeWidth="2" />}
                    {currentSnap.type === 'center' && <circle id="osnap-center-marker" cx={snapSvgX} cy={snapSvgY} r="5.5" fill="none" stroke="#22c55e" strokeWidth="2" />}
                    <g transform={`translate(${snapSvgX + 9}, ${snapSvgY - 9})`}>
                      <rect x="-2" y="-8" width={currentSnap.type === 'endpoint' ? 50 : currentSnap.type === 'midpoint' ? 46 : 42} height="13" rx="2" fill="rgba(2, 44, 34, 0.9)" stroke="#22c55e" strokeWidth="1" />
                      <text x="2" y="2" fill="#22c55e" fontSize="8.5" fontFamily="monospace" fontWeight="bold">{currentSnap.type === 'endpoint' ? 'Endpoint' : currentSnap.type === 'midpoint' ? 'Midpoint' : 'Center'}</text>
                    </g>
                  </g>
                );
              })()}
        ...
        <div className="absolute bottom-3 right-3 bg-slate-900/90 backdrop-blur border border-slate-800 rounded-lg px-3 py-1.5 text-xs shadow-lg flex items-center gap-3 font-mono">
          ...
          {cursorWorld ? (
            <div className="flex items-center gap-2">
              <span className="text-rose-400 font-semibold">X: {cursorWorld.x.toFixed(cursorWorld.x % 1 === 0 ? 0 : 2)} mm</span><span className="text-slate-600">|</span>
              <span className="text-emerald-400 font-semibold">Y: {cursorWorld.y.toFixed(cursorWorld.y % 1 === 0 ? 0 : 2)} mm</span>
            </div>
          ) : (
            <span className="text-slate-500 italic">Hover canvas</span>
          )}
          {currentSnap && (
            <><div className="text-slate-600">|</div><div className="flex items-center gap-1.5 text-emerald-400 font-semibold font-mono text-[11px]"><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse" /><span className="uppercase">SNAP: {currentSnap.type}</span></div></>
          )}
          {selectedEntityIds.length > 0 && (
            <><div className="text-slate-600">|</div><div className="flex items-center gap-1.5 text-sky-400 font-semibold font-mono text-[11px]"><span className="w-1.5 h-1.5 rounded-full bg-sky-400 animate-pulse" /><span>SELECTED: {selectedEntityIds.length}</span></div></>
          )}
          <div className="text-slate-600">|</div>
          <div className="text-slate-300">Zoom: <span className="text-amber-400 font-semibold">{Math.round((scale / 2.6) * 100)}%</span></div>
        ...
        {selectedEntities.length > 0 && selectedEntity && !isDrawing && (
          <div className="absolute bottom-3 left-3 bg-slate-900/90 backdrop-blur border border-slate-700 rounded-lg p-3 text-xs shadow-xl max-w-xs animate-in fade-in">
            <div className="flex items-center justify-between pb-1.5 border-b border-slate-800">
              <span className="font-semibold text-sky-400 uppercase">{selectedEntities.length > 1 ? `${selectedEntities.length} Entities Selected` : `${selectedEntity.type} Entity`}</span>
              <span className="font-mono text-[10px] text-slate-400">{selectedEntities.length === 1 ? `ID: ${selectedEntity.id}` : 'Shift+Click Multi'}</span>
            </div>
            {selectedEntities.length === 1 ? (
              <div className="grid grid-cols-2 gap-x-3 gap-y-1 mt-2 text-[11px] text-slate-300">
                <div>Layer: <span className="text-white font-mono">{selectedEntity.layer}</span></div>
                <div>State: <span className="text-emerald-400">{selectedEntity.state}</span></div>
                <div>Construction: <span className="text-purple-400">{selectedEntity.isConstruction ? 'Yes' : 'No'}</span></div>
                <div>Color: <span className="inline-block w-3 h-3 rounded-full align-middle ml-1" style={{ backgroundColor: selectedEntity.color || '#3b82f6' }} /></div>
              </div>
            ) : (
              <div className="mt-2 text-[11px] text-slate-300 space-y-1">
                <div className="text-slate-400 text-[10px]">Hold <kbd className="px-1 py-0.5 bg-slate-800 rounded font-mono text-slate-200">Shift</kbd> to add/remove</div>
                <div className="flex flex-wrap gap-1 max-h-20 overflow-y-auto pt-1">
                  {selectedEntities.map((ent) => (
                    <span key={ent.id} className="px-1.5 py-0.5 rounded bg-sky-950/80 border border-sky-800 text-[10px] text-sky-300 font-mono">
            ...
            <div className="mt-2.5 pt-2 border-t border-slate-800 flex justify-end">
              <button onClick={() => { selectedEntityIds.forEach((id) => deleteEntity(targetSketchId, id)); setSelectedEntityIds([]); }} className="flex items-center gap-1 px-2 py-1 rounded bg-rose-950/80 text-rose-300 border border-rose-800 hover:bg-rose-900 transition-colors text-[10px]">
